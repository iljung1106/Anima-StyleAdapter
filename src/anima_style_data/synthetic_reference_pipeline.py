...
def cache_lora_teacher_expansion_dual_query_tokens(
    config: dict[str, Any], destination: Path
) -> dict[str, Any]:
    return _cache_synthetic_dual_query_tokens(
        config, destination, config_key="lora_teacher_references_512_expansion"
    )
...
def cache_v2d_diverse_mixture_dual_query_tokens(
    config: dict[str, Any], destination: Path
) -> dict[str, Any]:
    return _cache_synthetic_dual_query_tokens(
        config,
...
def cache_external_lora_dual_query_tokens(
    config: dict[str, Any], destination: Path
) -> dict[str, Any]:
    return _cache_synthetic_dual_query_tokens(
        config, destination, config_key="external_civitai_lora_references"
    )
