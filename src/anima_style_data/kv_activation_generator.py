    ...
    try:
        for micro_step in range(
            ...
            if _direct_delta_flow_due(relative_step, human_flow):
                ...
                teacher_anchor_loss = latents.new_zeros((), dtype=torch.float32)
                teacher_anchor_huber = latents.new_zeros((), dtype=torch.float32)
                teacher_anchor_cosine = latents.new_zeros((), dtype=torch.float32)
                weighted_teacher_anchor = latents.new_zeros((), dtype=torch.float32)
                noise = torch.randn(
                    latents.shape,
                    device=device,
                    dtype=latents.dtype,
                ...
                if teacher_anchor_weight:
                    ...
                    with torch.autocast("cuda", dtype=torch.bfloat16):
                        anchor_style = reader(
                            flow_references, flow_main_mask
                        ).tokens
                        flow_injector.set_style(anchor_style)
                        ...
                        weighted_teacher_anchor = (
                            float(human_flow.get("flow_loss_weight", 1.0))
                            * teacher_anchor_weight
                            * teacher_anchor_loss
                        )
                    (
                        weighted_teacher_anchor / accumulation_steps
                    ).backward()
                    del (
                        anchor_style, anchor_prediction, anchor_student,
                        anchor_context, anchor_noisy, anchor_base,
                        anchor_teacher, anchor_padding,
                    )
                block_dropout = float(
                ...
                loss = (
                    weighted_flow.detach()
                    + weighted_ranking.detach()
                    + weighted_teacher_anchor.detach()
                )
                ...
                if teacher_anchor_weight:
                    ...
                    running["human_flow/teacher_anchor_weighted"].append(
                        float(weighted_teacher_anchor.detach())
                    )
...
def train_scheduled_expert_kv_teacher_flow_hybrid_b3_1k(
    config: dict[str, Any], destination: Path
) -> dict[str, Any]:
    return train_scheduled_direct_reference_kv_delta_320(
        config,
        destination,
        config_key="kv_reference_expert_kv_teacher_flow_hybrid_b3_1k",
        sample_config_key=(
...
def sample_expert_kv_teacher_flow_hybrid_b3_1k(
    config: dict[str, Any], destination: Path
) -> dict[str, Any]:
    return sample_direct_reference_kv_delta_320(
        config,
...
def train_scheduled_expert_kv_human_teacher_flow_1k(
    config: dict[str, Any], destination: Path
) -> dict[str, Any]:
    return train_scheduled_direct_reference_kv_delta_320(
        config,
        destination,
        config_key="kv_reference_expert_kv_human_teacher_flow_1k",
        sample_config_key="kv_reference_expert_kv_human_teacher_flow_1k_sample",
    )


def sample_expert_kv_human_teacher_flow_1k(
    config: dict[str, Any], destination: Path
) -> dict[str, Any]:
    return sample_direct_reference_kv_delta_320(
        config,
